...
public class AclFeedDemo {
     ...
     */
    private static void printAclList(CalendarService service) throws ServiceException, IOException {
        CalendarFeed calendarFeed = service.getFeed(metafeedUrl, CalendarFeed.class);
     ...
     * @throws IOException Error communicating with the server.
     */
    private static void addAccessControl(CalendarService service, String userEmail, AclRole role) throws ServiceException, IOException {
        ...
        Log.e("Calendar", "Added user to access control list:");
        Log.e("Calendar", "\tScope: Type=" + insertedEntry.getScope().getType() + " (" + insertedEntry.getScope().getValue() + ")");
        Log.e("Calendar", "\tRole: " + insertedEntry.getRole().getValue());
    }
     ...
     * @throws IOException Error communicating with the server.
     */
    private static void updateAccessControl(CalendarService service, String userEmail, AclRole newRole) throws ServiceException, IOException {
        ...
        for (AclEntry aclEntry : aclFeed.getEntries()) {
            if (userEmail.equals(aclEntry.getScope().getValue())) {
                ...
                Log.e("Calendar", "Updated user's access control:");
                Log.e("Calendar", "\tScope: Type=" + updatedEntry.getScope().getType() + " (" + updatedEntry.getScope().getValue() + ")");
                Log.e("Calendar", "\tRole: " + updatedEntry.getRole().getValue());
     ...
     * @throws IOException Error communicating with the server.
     */
    private static void deleteAccessControl(CalendarService service, String userEmail) throws ServiceException, IOException {
        AclFeed aclFeed = service.getFeed(aclFeedUrl, AclFeed.class);

        for (AclEntry aclEntry : aclFeed.getEntries()) {
            if (userEmail.equals(aclEntry.getScope().getValue())) {
                aclEntry.delete();
                Log.e("Calendar", "Deleted " + userEmail + "'s access control.");
    ...
    private static void usage() {
        Log.e("Calendar", "Syntax: AclFeedDemo <username> <password>" + " [userToShareWith]");
