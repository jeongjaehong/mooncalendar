...
public class EventFeedPartialDemo {
    ...
    private static final String CALENDAR_FEEDS_PREFIX = "http://www.google.com/calendar/feeds/";
    ...
    /** WidgetService instance to talk to calendar server */
     ...
     */
    private void printAttendeeStatus(String uname) throws IOException, ServiceException {

        String eventsFeedUrl = CALENDAR_FEEDS_PREFIX + uname + "/private/composite";
        ...
        for (CalendarEventEntry event : events.getEntries()) {
            ...
            OUT.println(eventId + ": " + event.getTitle().getPlainText() + ": (" + (attendeeStatus != null ? attendeeStatus : "no status") + ")");
        }
    }
     ...
     */
    private void updateAttendeeStatus(String uname) throws IOException, ServiceException {
        ...
        String eventEntryUrl = CALENDAR_FEEDS_PREFIX + uname + "/private/full/" + eventId;
        // Selection criteria to fetch only the attendee status of specified user.
        String selectAttendee = "@gd:etag,title,gd:who[@email='" + uname + "']";

        Query partialQuery = new Query(new URL(eventEntryUrl));
        partialQuery.setFields(selectAttendee);

        //EventEntry event = service.getEntry(partialQuery.getFeedUrl(), CalendarEventEntry.class);
        ...
        //OUT.println(event.getTitle().getPlainText() + " updated to: " + event.getParticipants().get(0).getAttendeeStatus());
    }
    ...
    public static void main(String args[]) throws IOException, ServiceException {
        ...
        CalendarService myService = new CalendarService("gdata-CalendarPartialDemo");
